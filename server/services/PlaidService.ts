			...
			try {
				...
				for (const transaction of transactions) {
					...
					if (matchingTransactions.length === 1) {
						...
						// If plaid identifies a merchant, upsert it
						// For now, all merchants are added by plaid alone
						let merchant_id;
						if (asAny(transaction).merchant_entity_id && !match.merchant_id) {
							const merchant = await MerchantDao.upsertMerchant({
								plaid_merchant_id: asAny(transaction).merchant_entity_id,
							}, {
						...
						await TransactionDao.updateTransaction(workspace_id, matchingTransactions[0].transaction_id, {
