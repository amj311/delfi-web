import type { ParentCategory } from "./Category";


export const nestedCategories: ParentCategory[] = [
    {
        name: "Auto & Transport",
        category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75",
		type: "EXPENSE",
        children: [
            {
                name: "Registration",
                category_id: "62ce18c1-8dc2-44a4-9ec7-cc355f59018b",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            },
            {
                name: "Auto Insurance",
                category_id: "a9701bf6-4ad9-4ef2-8b3f-acddd562e6e6",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            },
            {
                name: "Auto Payment",
                category_id: "8adb79fa-2857-41a5-af14-73e6a3b471aa",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            },
            {
                name: "Fuel",
                category_id: "ab0bb7ab-0367-4d7a-adbb-30846f5ec897",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            },
            {
                name: "Parking",
                category_id: "92d6b0df-8fcc-40a2-9591-cf03af9dc723",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            },
            {
                name: "Public Transportation",
                category_id: "4acd8f6c-a108-41f7-bb74-44f342008f5e",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            },
            {
                name: "Service & Parts",
                category_id: "68a5922f-88cb-4f93-abac-194ab7c5193a",
				type: "EXPENSE",
                parent_category_id: "7d2babb7-1065-4a7c-8552-67ba2f185f75"
            }
        ]
    },
    {
        name: "Bills & Utilities",
        category_id: "ba16a566-cf88-4a3f-8b7c-877d3357d123",
		type: "EXPENSE",
        children: [
            {
                name: "Home Phone",
                category_id: "8ce8b6b3-3308-4b84-9d12-ce386b829d75",
				type: "EXPENSE",
                parent_category_id: "ba16a566-cf88-4a3f-8b7c-877d3357d123"
            },
            {
                name: "Internet",
                category_id: "f1ac1ce7-b6b2-4fd1-abbc-0f624c7f7223",
				type: "EXPENSE",
                parent_category_id: "ba16a566-cf88-4a3f-8b7c-877d3357d123"
            },
            {
                name: "Mobile Phone",
                category_id: "2815a130-ebff-4aa1-94b9-1a350d1e8465",
				type: "EXPENSE",
                parent_category_id: "ba16a566-cf88-4a3f-8b7c-877d3357d123"
            },
            {
                name: "Television",
                category_id: "2734e341-e0c9-40c7-bad7-98db8e1e32f0",
				type: "EXPENSE",
                parent_category_id: "ba16a566-cf88-4a3f-8b7c-877d3357d123"
            },
            {
                name: "Utilities",
                category_id: "c1b2a2ca-c82b-4efd-9bbe-31928173eb9a",
				type: "EXPENSE",
                parent_category_id: "ba16a566-cf88-4a3f-8b7c-877d3357d123"
            }
        ]
    },
    {
        name: "Business Services",
        category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5",
		type: "EXPENSE",
        children: [
            {
                name: "Software",
                category_id: "42e60fe3-a98c-4270-86ad-4803ae25de24",
				type: "EXPENSE",
                parent_category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5"
            },
            {
                name: "Advertising",
                category_id: "50c047e5-d08a-424f-b65b-b6cd66093584",
				type: "EXPENSE",
                parent_category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5"
            },
            {
                name: "Legal",
                category_id: "9e1724be-d700-4296-a918-f578860c2f0e",
				type: "EXPENSE",
                parent_category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5"
            },
            {
                name: "Office Supplies",
                category_id: "e5cbbf97-6ef6-46d6-be78-ecfe196fdd9c",
				type: "EXPENSE",
                parent_category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5"
            },
            {
                name: "Printing",
                category_id: "57c39298-5057-4aed-b4ef-f6b6e0341a06",
				type: "EXPENSE",
                parent_category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5"
            },
            {
                name: "Shipping",
                category_id: "04a4d5e0-b664-463a-adf9-4c5cb23089b9",
				type: "EXPENSE",
                parent_category_id: "e8ddbfdf-7b34-41dd-90cb-134ef3c737f5"
            }
        ]
    },
    {
        name: "Education",
        category_id: "1ebd41dc-dee5-49da-8e67-4937d4ceb29b",
		type: "EXPENSE",
        children: [
            {
                name: "Books & Supplies",
                category_id: "8c04591a-1ab8-4e08-9f01-83ae9d413ca0",
				type: "EXPENSE",
                parent_category_id: "1ebd41dc-dee5-49da-8e67-4937d4ceb29b"
            },
            {
                name: "Student Loan",
                category_id: "b2d285a2-59a7-469d-9bbc-7d3e1095c193",
				type: "EXPENSE",
                parent_category_id: "1ebd41dc-dee5-49da-8e67-4937d4ceb29b"
            },
            {
                name: "Tuition",
                category_id: "fd4e361a-23b0-4492-9768-2e8235a7c5f6",
				type: "EXPENSE",
                parent_category_id: "1ebd41dc-dee5-49da-8e67-4937d4ceb29b"
            }
        ]
    },
    {
        name: "Entertainment",
        category_id: "3ba0c04c-970d-4702-b892-7fbba9cc724d",
		type: "EXPENSE",
        children: [
            {
                name: "Amusement",
                category_id: "312482e9-9aa0-4710-a312-4a5dd308dedb",
				type: "EXPENSE",
                parent_category_id: "3ba0c04c-970d-4702-b892-7fbba9cc724d"
            },
            {
                name: "Arts",
                category_id: "ee167832-1262-4caf-bb78-4a6e987cd172",
				type: "EXPENSE",
                parent_category_id: "3ba0c04c-970d-4702-b892-7fbba9cc724d"
            },
            {
                name: "Movies & DVDs",
                category_id: "15510c92-62d3-49bc-8bdf-0891c1097dcf",
				type: "EXPENSE",
                parent_category_id: "3ba0c04c-970d-4702-b892-7fbba9cc724d"
            },
            {
                name: "Music",
                category_id: "58198b08-ae3b-4a87-bc73-1504647e578e",
				type: "EXPENSE",
                parent_category_id: "3ba0c04c-970d-4702-b892-7fbba9cc724d"
            },
            {
                name: "Newspapers & Magazines",
                category_id: "bd4c5e34-27ca-4652-a422-ec7673d90cc7",
				type: "EXPENSE",
                parent_category_id: "3ba0c04c-970d-4702-b892-7fbba9cc724d"
            }
        ]
    },
    {
        name: "Fees & Charges",
        category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff",
		type: "EXPENSE",
        children: [
            {
                name: "ATM Fee",
                category_id: "5d0b38c6-cd30-446b-824a-a5e1c137663a",
				type: "EXPENSE",
                parent_category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff"
            },
            {
                name: "Banking Fee",
                category_id: "6e20d2ed-f8f0-4562-ae37-7881e6560a62",
				type: "EXPENSE",
                parent_category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff"
            },
            {
                name: "Finance Charge",
                category_id: "40676754-e08b-4928-826f-a23961a2a042",
				type: "EXPENSE",
                parent_category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff"
            },
            {
                name: "Late Fee",
                category_id: "5461213e-ffd5-464b-b3c4-bc2f6fc957cb",
				type: "EXPENSE",
                parent_category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff"
            },
            {
                name: "Service Fee",
                category_id: "b4e435b8-d774-407f-afeb-5d79e4288f5f",
				type: "EXPENSE",
                parent_category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff"
            },
            {
                name: "Trade Commissions",
                category_id: "b6b62d72-1cbc-4087-b928-6163abcff64a",
				type: "EXPENSE",
                parent_category_id: "4a628c14-129a-4f23-9d5f-0a9096976bff"
            }
        ]
    },
    {
        name: "Financial",
        category_id: "db921da6-a7df-4c82-8c40-1dba66385487",
		type: "EXPENSE",
        children: [
            {
                name: "Financial Advisor",
                category_id: "69c87283-4a58-4aac-8cb3-c67d26e121a0",
				type: "EXPENSE",
                parent_category_id: "db921da6-a7df-4c82-8c40-1dba66385487"
            },
            {
                name: "Life Insurance",
                category_id: "7f989aae-f1b3-408e-95fb-d7b90b88be5f",
				type: "EXPENSE",
                parent_category_id: "db921da6-a7df-4c82-8c40-1dba66385487"
            }
        ]
    },
    {
        name: "Food & Dining",
        category_id: "94ab8c8e-7c79-4277-9953-a0f264c040b4",
		type: "EXPENSE",
        children: [
            {
                name: "Treats and Sweets",
                category_id: "3d8d6a00-d0d4-4908-a8c1-ff3d830d3ad3",
				type: "EXPENSE",
                parent_category_id: "94ab8c8e-7c79-4277-9953-a0f264c040b4"
            },
            {
                name: "Alcohol & Bars",
                category_id: "4cc55ded-9014-4863-a0d6-4d084fea3ea6",
				type: "EXPENSE",
                parent_category_id: "94ab8c8e-7c79-4277-9953-a0f264c040b4"
            },
            {
                name: "Coffee Shops",
                category_id: "ef5e7110-72f7-4fb8-a650-7e3db4f406f6",
				type: "EXPENSE",
                parent_category_id: "94ab8c8e-7c79-4277-9953-a0f264c040b4"
            },
            {
                name: "Fast Food",
                category_id: "4bca50a9-190f-44eb-94d2-e02df52ca312",
				type: "EXPENSE",
                parent_category_id: "94ab8c8e-7c79-4277-9953-a0f264c040b4"
            },
            {
                name: "Restaurants",
                category_id: "6f58cb2e-f20e-47c1-8584-078d419c4e30",
				type: "EXPENSE",
                parent_category_id: "94ab8c8e-7c79-4277-9953-a0f264c040b4"
            }
        ]
    },
    {
        name: "Gifts & Donations",
        category_id: "e8bca570-9908-4e75-93ce-bb2234377720",
		type: "EXPENSE",
        children: [
            {
                name: "Christmas",
                category_id: "c9188777-62fb-4411-b4a1-9ed1e5447212",
				type: "EXPENSE",
                parent_category_id: "e8bca570-9908-4e75-93ce-bb2234377720"
            },
            {
                name: "Birthday",
                category_id: "22fa90b8-f3f3-4a8c-922b-6b8835e977f8",
				type: "EXPENSE",
                parent_category_id: "e8bca570-9908-4e75-93ce-bb2234377720"
            },
            {
                name: "Charity",
                category_id: "e642de71-002a-4afc-a660-d155de11b627",
				type: "EXPENSE",
                parent_category_id: "e8bca570-9908-4e75-93ce-bb2234377720"
            },
            {
                name: "Gift",
                category_id: "594b69f1-cef7-4f49-b275-174d00ecece0",
				type: "EXPENSE",
                parent_category_id: "e8bca570-9908-4e75-93ce-bb2234377720"
            },
			{
				name: "Tithing",
				category_id: 'd8f0b1c2-3e4a-4f5c-9b6e-7d8f0b1c2e4a',
				type: "EXPENSE",
				parent_category_id: "e8bca570-9908-4e75-93ce-bb2234377720",
			},
			{
				name: "Fast Offering",
				category_id: 'd8f0b1c2-3e4a-4f5c-9b6e-7d8f0b1c2e4b',
				type: "EXPENSE",
				parent_category_id: "e8bca570-9908-4e75-93ce-bb2234377720",
			}
        ]
    },
    {
        name: "Health & Fitness",
        category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244",
		type: "EXPENSE",
        children: [
            {
                name: "Dentist",
                category_id: "c795f728-f94e-48d1-9cd2-77221bb50a1b",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            },
            {
                name: "Doctor",
                category_id: "a4524662-e6f5-4bfe-9e4e-4fbec5814628",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            },
            {
                name: "Eyecare",
                category_id: "b19e46a3-ce8f-4a96-b1d3-28cdd313fb85",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            },
            {
                name: "Gym",
                category_id: "bdc8aade-ea65-4e20-bb4a-7c40ebadc0a1",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            },
            {
                name: "Health Insurance",
                category_id: "36deecac-e079-48ff-9b0d-b2ff3f252303",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            },
            {
                name: "Pharmacy",
                category_id: "33736d95-667c-4f8d-8434-487cb9caff3d",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            },
            {
                name: "Sports",
                category_id: "05c69d5a-4472-4dad-bbe7-35fa673afc17",
				type: "EXPENSE",
                parent_category_id: "2386892c-50e5-4ba0-a6bd-5f143f5ef244"
            }
        ]
    },
    {
        name: "Home",
        category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca",
		type: "EXPENSE",
        children: [
            {
                name: "Furnishings",
                category_id: "db218c86-5d58-40c9-9695-f179a3d44f2d",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            },
            {
                name: "Home Improvement",
                category_id: "48aa9eed-143a-44aa-9352-8dff4b3c2d68",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            },
            {
                name: "Home Insurance",
                category_id: "1233f9f8-b986-4934-a58c-edaf9a062a35",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            },
            {
                name: "Home Services",
                category_id: "b7ce7783-9da9-4698-95f9-7d615174216b",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            },
            {
                name: "Home Supplies",
                category_id: "50dbd848-3862-4a58-a18b-cd366db603f0",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            },
            {
                name: "Lawn & Garden",
                category_id: "b812dac6-6a37-429e-a1c1-ede2c7f99b63",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            },
            {
                name: "Mortgage & Rent",
                category_id: "98bd1be3-fcde-4fbe-af1b-7178315a25da",
				type: "EXPENSE",
                parent_category_id: "2f850454-2fdb-45fd-8866-95ade0c027ca"
            }
        ]
    },
    {
        name: "Income",
        category_id: "dc3c1a38-1b99-41d5-b9b8-a685c6eacaba",
		type: "INCOME",
        children: [
            {
                name: "Interest Income",
                category_id: "f26d6a29-64e9-443f-b2c3-81d2b9921142",
				type: "INCOME",
                parent_category_id: "dc3c1a38-1b99-41d5-b9b8-a685c6eacaba"
            },
            {
                name: "Paycheck",
                category_id: "881890d0-64d9-436b-a8d4-982b11276e68",
				type: "INCOME",
                parent_category_id: "dc3c1a38-1b99-41d5-b9b8-a685c6eacaba"
            },
            {
                name: "Reimbursement",
                category_id: "257d286a-6593-41ae-a5a5-55e961a47998",
				type: "INCOME",
                parent_category_id: "dc3c1a38-1b99-41d5-b9b8-a685c6eacaba"
            },
            {
                name: "Rental Income",
                category_id: "2114648b-d34e-42c6-9d62-41b4eaf73571",
				type: "INCOME",
                parent_category_id: "dc3c1a38-1b99-41d5-b9b8-a685c6eacaba"
            },
            {
                name: "Returned Purchase",
                category_id: "ceb21839-7cc0-4534-986d-4ecaac759a46",
				type: "INCOME",
                parent_category_id: "dc3c1a38-1b99-41d5-b9b8-a685c6eacaba"
            }
        ]
    },
    {
        name: "Investments",
        category_id: "f549c254-cd52-4668-bd24-b3f131eb1559",
		type: "EXPENSE",
        children: [
            {
                name: "Buy",
                category_id: "53b6de9b-0761-42e0-b724-e5badf1a11fd",
				type: "EXPENSE",
                parent_category_id: "f549c254-cd52-4668-bd24-b3f131eb1559"
            },
            {
                name: "Deposit",
                category_id: "05c3c8b1-002f-424b-9ca0-556f5ddbb21b",
				type: "EXPENSE",
                parent_category_id: "f549c254-cd52-4668-bd24-b3f131eb1559"
            },
            {
                name: "Dividend & Cap Gains",
                category_id: "a06575b1-b94f-4f08-91c7-27b58a1ef6ab",
				type: "EXPENSE",
                parent_category_id: "f549c254-cd52-4668-bd24-b3f131eb1559"
            },
            {
                name: "Sell",
                category_id: "11bda97c-7de0-4004-89fc-56191b2e1b3a",
				type: "EXPENSE",
                parent_category_id: "f549c254-cd52-4668-bd24-b3f131eb1559"
            },
            {
                name: "Withdrawal",
                category_id: "b0ef4648-5801-4745-8fd6-9c23c0d8c23a",
				type: "EXPENSE",
                parent_category_id: "f549c254-cd52-4668-bd24-b3f131eb1559"
            }
        ]
    },
    {
        name: "Kids",
        category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7",
		type: "EXPENSE",
        children: [
            {
                name: "Allowance",
                category_id: "aea68813-cb83-472e-9ecd-387e2182ad37",
				type: "EXPENSE",
                parent_category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7"
            },
            {
                name: "Baby Supplies",
                category_id: "ee69d893-35b3-4a66-9d54-a4176797e5dc",
				type: "EXPENSE",
                parent_category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7"
            },
            {
                name: "Babysitter & Daycare",
                category_id: "7d9334c5-7942-4adc-844b-dc17c6352aac",
				type: "EXPENSE",
                parent_category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7"
            },
            {
                name: "Child Support",
                category_id: "887e74ee-b201-47da-9f3c-378ade64559c",
				type: "EXPENSE",
                parent_category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7"
            },
            {
                name: "Kids Activities",
                category_id: "030d6853-7ef8-4aa2-b574-4f48a83a1e84",
				type: "EXPENSE",
                parent_category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7"
            },
            {
                name: "Toys",
                category_id: "7ca8b829-b60d-4a99-8f7e-28f9760afc14",
				type: "EXPENSE",
                parent_category_id: "66003f95-7204-43ff-b0ed-cc76c62b20e7"
            }
        ]
    },
    {
        name: "Personal Care",
        category_id: "a3b4c3c2-340c-48be-8d85-7b0f919acf3d",
		type: "EXPENSE",
        children: [
            {
                name: "Hair",
                category_id: "aad5343d-75b9-481b-bbeb-4d45e445f120",
				type: "EXPENSE",
                parent_category_id: "a3b4c3c2-340c-48be-8d85-7b0f919acf3d"
            },
            {
                name: "Laundry",
                category_id: "f0ed5497-ce37-41ad-91a3-4fd9675ac609",
				type: "EXPENSE",
                parent_category_id: "a3b4c3c2-340c-48be-8d85-7b0f919acf3d"
            },
            {
                name: "Spa & Massage",
                category_id: "253feafa-5ae0-4f9e-9b60-336e9949c580",
				type: "EXPENSE",
                parent_category_id: "a3b4c3c2-340c-48be-8d85-7b0f919acf3d"
            }
        ]
    },
    {
        name: "Pets",
        category_id: "e22ca555-02b7-4c8b-8ca5-4eca7e554a6d",
		type: "EXPENSE",
        children: [
            {
                name: "Pet Food & Supplies",
                category_id: "fae9177b-c986-4db7-b98f-feb71e266a9d",
				type: "EXPENSE",
                parent_category_id: "e22ca555-02b7-4c8b-8ca5-4eca7e554a6d"
            },
            {
                name: "Pet Grooming",
                category_id: "b83703ff-3f6d-4565-bfbe-5c76ebf442d6",
				type: "EXPENSE",
                parent_category_id: "e22ca555-02b7-4c8b-8ca5-4eca7e554a6d"
            },
            {
                name: "Veterinary",
                category_id: "ddee763e-dc14-4f77-ba75-105adadcba4b",
				type: "EXPENSE",
                parent_category_id: "e22ca555-02b7-4c8b-8ca5-4eca7e554a6d"
            }
        ]
    },
    {
        name: "Shopping",
        category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3",
		type: "EXPENSE",
        children: [
            {
                name: "Books",
                category_id: "c4ec5320-bd66-4ad0-a8f9-0c90b9811485",
				type: "EXPENSE",
                parent_category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3"
            },
            {
                name: "Clothing",
                category_id: "b83f3fad-f06b-4052-b35b-01d261009c50",
				type: "EXPENSE",
                parent_category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3"
            },
            {
                name: "Electronics & Software",
                category_id: "ddd1f2b7-f087-49fe-86dc-318fb574dd9f",
				type: "EXPENSE",
                parent_category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3"
            },
            {
                name: "Hobbies",
                category_id: "3a5d255c-4210-430c-9522-528edaf60c57",
				type: "EXPENSE",
                parent_category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3"
            },
            {
                name: "Sporting Goods",
                category_id: "c28290c3-5900-46e8-8e79-10ca31cd0e42",
				type: "EXPENSE",
                parent_category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3"
            },
            {
                name: "Groceries",
                category_id: "93d35092-b4a7-424e-a86b-c3d4ff99e4e5",
				type: "EXPENSE",
                parent_category_id: "0e469f49-c1a1-4f9f-98a1-ce55dc7c5bb3"
            }
        ]
    },
    {
        name: "Taxes",
        category_id: "c1fcd3bc-0730-4e18-94f4-5948a0308cde",
		type: "EXPENSE",
        children: [
            {
                name: "Federal Tax",
                category_id: "cbae3766-7d64-49f6-92de-19ad1c576ba5",
				type: "EXPENSE",
                parent_category_id: "c1fcd3bc-0730-4e18-94f4-5948a0308cde"
            },
            {
                name: "Local Tax",
                category_id: "e3970bf9-efae-496b-bd53-4f98a8c8163b",
				type: "EXPENSE",
                parent_category_id: "c1fcd3bc-0730-4e18-94f4-5948a0308cde"
            },
            {
                name: "Property Tax",
                category_id: "22090305-6510-4acd-8b2c-440a200ba311",
				type: "EXPENSE",
                parent_category_id: "c1fcd3bc-0730-4e18-94f4-5948a0308cde"
            },
            {
                name: "Sales Tax",
                category_id: "2c65aa58-cba0-49c8-a813-56b8c5f8df50",
				type: "EXPENSE",
                parent_category_id: "c1fcd3bc-0730-4e18-94f4-5948a0308cde"
            },
            {
                name: "State Tax",
                category_id: "4d4b349f-f31c-4d81-b1cf-39476126fd0f",
				type: "EXPENSE",
                parent_category_id: "c1fcd3bc-0730-4e18-94f4-5948a0308cde"
            }
        ]
    },
    {
        name: "Transfer",
        category_id: "8de5c54d-f8e1-4127-89c2-ee91d2e399ce",
		type: "TRANSFER",
        children: [
            {
                name: "Refund",
                category_id: "9a52efe9-b3e7-4fe1-8547-2ef477d4e34d",
				type: "TRANSFER",
                parent_category_id: "8de5c54d-f8e1-4127-89c2-ee91d2e399ce"
            },
            {
                name: "Credit Card Payment",
                category_id: "4b45bab9-fe7e-4224-a9d5-8738dd0f56a7",
				type: "TRANSFER",
                parent_category_id: "8de5c54d-f8e1-4127-89c2-ee91d2e399ce"
            },
            {
                name: "Transfer for Cash Spending",
                category_id: "58685d03-42d9-416e-ad98-ebd0d0072e40",
				type: "TRANSFER",
                parent_category_id: "8de5c54d-f8e1-4127-89c2-ee91d2e399ce"
            },
            {
                name: "Mortgage Payment",
                category_id: "5826e87d-17e4-4ade-84bc-8e2a2309792d",
				type: "TRANSFER",
                parent_category_id: "8de5c54d-f8e1-4127-89c2-ee91d2e399ce"
            }
        ]
    },
    {
        name: "Travel",
        category_id: "5bced943-52e9-46bd-9663-2e68fa1b3aba",
		type: "EXPENSE",
        children: [
            {
                name: "Hotel & Lodging",
                category_id: "c199d55c-6e60-4e6a-8b15-675d6909db7b",
				type: "EXPENSE",
                parent_category_id: "5bced943-52e9-46bd-9663-2e68fa1b3aba"
            },
            {
                name: "Flights",
                category_id: "0fffae2e-9c68-432f-9ac2-c2dccbe1d13c",
				type: "EXPENSE",
                parent_category_id: "5bced943-52e9-46bd-9663-2e68fa1b3aba"
            },
            {
                name: "Transportation",
                category_id: "8c72592c-d453-46ac-9a2f-a9ea1570c1b9",
				type: "EXPENSE",
                parent_category_id: "5bced943-52e9-46bd-9663-2e68fa1b3aba"
            },
            {
                name: "Activities",
                category_id: "241b4aaa-4669-44f0-857f-a05a35540d34",
				type: "EXPENSE",
                parent_category_id: "5bced943-52e9-46bd-9663-2e68fa1b3aba"
            },
            {
                name: "Food",
                category_id: "9dcfd6a1-1633-4fea-9479-2ed46a5504ae",
				type: "EXPENSE",
                parent_category_id: "5bced943-52e9-46bd-9663-2e68fa1b3aba"
            }
        ]
    },
    {
        name: "Uncategorized",
        category_id: "26993349-13a8-422b-beb2-fd3be37f596b",
		type: "EXPENSE",
        children: []
    }
];


export const flatCategoriesMap = Object.fromEntries(nestedCategories.flatMap(parent => ([
	[parent.name, parent],
	...(parent.children.map(child => [child.name, {
		...child,
		parent: parent.name
	}]))
])))