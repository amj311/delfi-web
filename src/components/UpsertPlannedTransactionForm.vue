...
import { BudgetTransactionType, RecurrenceType } from '../../delfi-core/models/Budget';
...
<template>
	...
	<div><label for="type">Type</label>
		<select id="type" v-model="state.data.type">
			<option v-for="ptype in BudgetTransactionType" :key="ptype">{{ptype}}</option>
		</select>
	</div>
	<div><label for="target_account">{{ state.data.type === BudgetTransactionType.TRANSFER ? 'From ' : '' }}Account</label>
	...
	<div v-if="state.data.type === BudgetTransactionType.TRANSFER">
